...
""",
    ...
    App(
        key="deluge", name="Deluge", category="media",
        vi="Trình tải BitTorrent nhẹ, điều khiển qua web và chạy nền trên máy chủ.",
        en="A light BitTorrent client driven from the browser and left running on the server.",
        website="https://deluge-torrent.org",
        image="linuxserver/deluge", fixed_tags=("latest",),
        container_port=8112,
        volumes=("config:/config", "${DOWNLOAD_DIR}:/downloads"),
        fields=(
    ...
    App(
        key="pyload", name="pyLoad", category="media",
        vi="Trình tải tệp từ các trang chia sẻ, nhận cả danh sách liên kết và giải nén sau khi xong.",
        en="Downloads files from one-click hosters, taking link lists and unpacking when done.",
        website="https://pyload.net",
        image="linuxserver/pyload-ng", fixed_tags=("latest",),
        container_port=8000,
        volumes=("config:/config", "${DOWNLOAD_DIR}:/downloads"),
        fields=(
            port("HTTP_PORT", "Cổng web", "Web port", 8060),
            text("DOWNLOAD_DIR", "Thư mục tải về", "Download folder", "/srv/downloads",
                 help_vi=MEDIA_HELP_VI, help_en=MEDIA_HELP_EN),
        ),
        environment={"PUID": "1000", "PGID": "1000", "TZ": "Asia/Ho_Chi_Minh"},
    ...
    App(
        key="jdownloader", name="JDownloader", category="media",
        vi="Trình tải nhiều luồng có hàng đợi, dùng qua màn hình ứng dụng phát lại trong trình duyệt.",
        en="A multi-threaded download manager with a queue, used through its desktop screen in the browser.",
        website="https://jdownloader.org",
        image="jlesage/jdownloader-2", fixed_tags=("latest",),
        container_port=5800,
        volumes=("config:/config", "${DOWNLOAD_DIR}:/output"),
        fields=(
            port("HTTP_PORT", "Cổng web", "Web port", 5800),
            text("DOWNLOAD_DIR", "Thư mục tải về", "Download folder", "/srv/downloads",
                 help_vi=MEDIA_HELP_VI, help_en=MEDIA_HELP_EN),
        ),
        environment={"USER_ID": "1000", "GROUP_ID": "1000", "TZ": "Asia/Ho_Chi_Minh"},
    ),
    App(
        key="stash", name="Stash", category="media",
        vi="Bộ sắp xếp thư viện video cá nhân: quét thư mục, gắn thẻ và xem ngay trong trình duyệt.",
        en="Organises a personal video library: scans folders, tags them and plays in the browser.",
        website="https://stashapp.cc",
        image="stashapp/stash", tag_pages=6, min_major=0,
        container_port=9999,
        volumes=("config:/root/.stash", "generated:/generated", "${MEDIA_DIR}:/data"),
        fields=(
            port("HTTP_PORT", "Cổng web", "Web port", 9999),
            text("MEDIA_DIR", "Thư mục video", "Video folder", "/srv/videos",
                 help_vi=MEDIA_HELP_VI, help_en=MEDIA_HELP_EN),
        ),
        environment={"STASH_STASH": "/data/"},
    ),
    App(
        key="calibre", name="Calibre", category="media",
        vi="Phần mềm quản lý sách điện tử đầy đủ, chuyển định dạng và phát sách cho máy đọc.",
        en="The full ebook manager: converts formats and serves the library to reading devices.",
        website="https://calibre-ebook.com",
        image="linuxserver/calibre", fixed_tags=("latest",),
        container_port=8080,
        volumes=("config:/config", "${LIBRARY_DIR}:/books"),
        fields=(
            port("HTTP_PORT", "Cổng web", "Web port", 8083),
            text("LIBRARY_DIR", "Thư mục sách", "Library folder", "/srv/books",
                 help_vi=MEDIA_HELP_VI, help_en=MEDIA_HELP_EN),
        ),
        environment={"PUID": "1000", "PGID": "1000", "TZ": "Asia/Ho_Chi_Minh"},
    ),
