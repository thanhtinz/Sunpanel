...
""",
    ...
    App(
        key="zwave-js-ui", name="Z-Wave JS UI", category="automation",
        vi="Cầu nối thiết bị Z-Wave sang MQTT kèm giao diện quản lý mạng lưới và cập nhật thiết bị.",
        en="Bridges Z-Wave devices to MQTT with a UI for managing the mesh and updating devices.",
        website="https://zwave-js.github.io/zwave-js-ui",
        image="zwavejs/zwave-js-ui", tag_pages=6, min_major=9,
        container_port=8091, volumes=("store:/usr/src/app/store",),
        fields=(port("HTTP_PORT", "Cổng web", "Web port", 8091,
                     help_vi="Que cắm Z-Wave phải được cấp cho container qua mục devices trong tệp compose.",
                     help_en="The Z-Wave stick must be passed to the container via devices in the compose file."),),
        environment={"TZ": "Asia/Ho_Chi_Minh"},
    ),
    App(
        key="spoolman", name="Spoolman", category="automation",
        vi="Sổ kho cuộn nhựa in 3D: trừ dần khối lượng còn lại theo từng bản in.",
        en="Keeps stock of 3D printing filament, subtracting what each print uses.",
        website="https://github.com/Donkie/Spoolman",
        image="ghcr.io/donkie/spoolman", registry="ghcr", min_major=0,
        container_port=8000, volumes=("data:/home/app/.local/share/spoolman",),
        fields=(port("HTTP_PORT", "Cổng web", "Web port", 7912),),
        environment={"TZ": "Asia/Ho_Chi_Minh"},
    ),
    App(
        key="traccar", name="Traccar", category="monitoring",
        vi="Máy chủ giám sát hành trình cho hơn hai nghìn loại thiết bị định vị, vẽ đường đi trên bản đồ.",
        en="A GPS tracking server for over two thousand device types, drawing routes on a map.",
        website="https://www.traccar.org",
        image="traccar/traccar", tag_suffix="-alpine", tag_pages=6, min_major=5,
        container_port=8082, volumes=("data:/opt/traccar/data", "logs:/opt/traccar/logs"),
        fields=(port("HTTP_PORT", "Cổng web", "Web port", 8082,
                     help_vi="Mỗi loại thiết bị định vị nghe ở một cổng riêng; mở thêm cổng trong tệp compose khi cần.",
                     help_en="Each tracker protocol listens on its own port; open more ports in the compose file as needed."),),
    ),
    App(
        key="beszel", name="Beszel", category="monitoring",
        vi="Bảng giám sát máy chủ gọn nhẹ: CPU, bộ nhớ, đĩa và container, kèm cảnh báo.",
        en="A lightweight server monitoring dashboard: CPU, memory, disk and containers, with alerts.",
        website="https://beszel.dev",
        image="henrygd/beszel", tag_pages=6, min_major=0,
        container_port=8090, volumes=("data:/beszel_data",),
        fields=(port("HTTP_PORT", "Cổng web", "Web port", 8090,
                     help_vi="Mỗi máy cần giám sát phải chạy thêm tác tử beszel-agent trỏ về địa chỉ này.",
                     help_en="Every monitored machine also runs a beszel-agent pointed at this address."),),
    ),
]
